			...
			<tr>
				...
				<select id="souceConfigId" name="souceConfigId" class="esayui-combobox"  style="width:150px;"    >
		           ...
		           <c:forEach var="config" items="${ configList }"   >
		                 <option value="${config.id}" title="${config.name}"  <c:if test="${ dataSynchronize.souceConfigId == config.id}"> selected </c:if>  >${config.ip}:${config.port} </option>
		           </c:forEach>
				</select>
				</td>
			 ...
			 <tr>
				...
				<td> 
				 <select id="targetConfigId" name="targetConfigId" class="esayui-combobox"  style="width:150px;"   >
				       <option value="" > </option>
		               <c:forEach var="config" items="${ configList }"   >
		                 <option value="${config.id}"  title="${config.name}" <c:if test="${dataSynchronize.targetConfigId == config.id}"> selected </c:if>  > ${config.ip}:${config.port} </option>
		              </c:forEach>
				</select>
				 </td>
				<td>&nbsp;&nbsp;&nbsp;&nbsp;目标库名：</td>
