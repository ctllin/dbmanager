        ...
        if (username != "" && username != null) {
            ...
            if (identifyingCode.equals("1") && !captcha.equals(cap)) {
                ...
                if (list.size() <= 0) {
                    ...
                    String status = (String)((Map)list.get(0)).get("status");
                    String expiration = (String)((Map)list.get(0)).get("expiration");
                    String permission = (String)((Map)list.get(0)).get("permission");
                    if ("1".equals(status)) {
                        message = "当前用户已禁用！";
                        map.put("error", message);
                        request.setAttribute("message", message);
