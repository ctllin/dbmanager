	   ...
	   <input id="id" name="id" type="hidden" value="${config.id }"   />
		<table class="formTable">
			 ...
			 <tr>
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-tip" plain="true" title=" MySql 默认端口为:3306 &#13;MariaDB默认端口为:3306 &#13;Oracle默认端口为:1521 &#13;PostgreSQL默认端口:5432 &#13;MSSQL默认端口为:1433 &#13;MongoDB默认端口:27017 &#13; "></a>
				</td>
			 </tr>
...
function configUpdate2(){  
	  ...
	  $.ajax({
                    ...
                    data: JSON.stringify( { 'id':id ,'name':name ,'databaseType':databaseType ,'databaseName':databaseName ,'ip':ip ,'port':port ,'userName':userName ,'password':password,'isdefault':isdefault} ),
                    datatype: "json", 
                   //成功返回之后调用的函数             
...
//测试连接
 function  testConn(){
	  ...
	  $.ajax({
            ...
            data: JSON.stringify({ 'databaseType':$("#databaseType").val(),'databaseName':$("#databaseName").val(),'userName':$("#userName").val(),'password':$("#password").val(),'port':$("#port").val(),'ip':$("#ip").val() } ),
            datatype: "json", 
            //成功返回之后调用的函数             
            success:function(data){
            	var status = data.status ;
